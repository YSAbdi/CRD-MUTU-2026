...
{
    public function guests(Request $request) { return Guest::query()->when($request->search, fn ($q,$s) => $q->where(fn($x) => $x->where('name','like',"%{$s}%")->orWhere('identity_number','like',"%{$s}%")->orWhere('phone','like',"%{$s}%")))->latest()->paginate(min((int)$request->integer('per_page',25),100)); }
    ...
    public function bookings(Request $request) { return Booking::with(['guest:id,name,identity_number','room:id,code,name'])->when($request->status,fn($q,$s)=>$q->where('status',$s))->when($request->from,fn($q,$d)=>$q->where('check_out','>=',$d))->when($request->to,fn($q,$d)=>$q->where('check_in','<=',$d))->latest('check_in')->paginate(min((int)$request->integer('per_page',25),100)); }
    public function storeBooking(Request $request) { $data=$request->validate(['guest_id'=>'required|exists:guests,id','room_id'=>'required|exists:rooms,id','check_in'=>'required|date|before:check_out','check_out'=>'required|date|after:check_in','notes'=>'nullable|string']); $booking=DB::transaction(function() use($data,$request){$room=Room::findOrFail($data['room_id']);abort_if($room->status!=='available',422,'Kamar tidak tersedia.');abort_if(Booking::overlapping($room->id,$data['check_in'],$data['check_out'])->exists(),422,'Kamar telah dipesan pada rentang tersebut.');$data+=['booking_code'=>'MUTU-'.Str::upper(Str::random(8)),'created_by'=>$request->user()->id];return Booking::create($data)->load(['guest','room','creator']);});if($booking->creator)$booking->creator->notify(new BookingStatusChanged($booking));return response()->json($booking,201); }
    public function updateBooking(Request $request, Booking $booking) { $data=$request->validate(['guest_id'=>'sometimes|exists:guests,id','room_id'=>'sometimes|exists:rooms,id','check_in'=>'sometimes|date|before:check_out','check_out'=>'sometimes|date|after:check_in','notes'=>'nullable|string']);$roomId=$data['room_id']??$booking->room_id;$from=$data['check_in']??$booking->check_in;$to=$data['check_out']??$booking->check_out;abort_if(Booking::where('id','!=',$booking->id)->overlapping($roomId,$from,$to)->exists(),422,'Perubahan bertabrakan dengan booking lain.');$booking->update($data);return $booking->fresh(['guest','room']); }
